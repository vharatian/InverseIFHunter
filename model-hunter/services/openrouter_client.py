    ...
    async def call_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Empty response - will retry (no two-pass, no reasoning-as-response)
                print(f"DEBUG OpenRouter: Empty response from {model}, will retry (attempt {attempt + 1}/{max_retries})")
