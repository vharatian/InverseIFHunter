    ...
    async def call_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                # For Qwen on Fireworks: if content is empty but we have reasoning,
                # continue to retry (no two-pass for Qwen)
                if reasoning and reasoning.strip() and not response_text.strip():
                    print(f"DEBUG Fireworks: Qwen has reasoning ({len(reasoning)} chars) but empty response, will retry")
