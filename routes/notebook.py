...
async def upload_notebook(request: Request, file: UploadFile = File(...), force_new: bool = False):
            ...
            if task_id:
                ...
                if existing:
                    return {
                        ...
                        "notebook": {"metadata": parsed.metadata, "filename": file.filename},
                    }
...
@router.get("/check-task-id/{task_id}")
async def check_task_id(task_id: str):
    """Check if any active session already uses this task ID."""
    matches = await redis_store.find_sessions_by_task_id(task_id)
    return {"exists": len(matches) > 0, "sessions": matches}
...
class CreateNotebookRequest(BaseModel):
    title: Optional[str] = "Trainer Notebook"
    trainer_email: Optional[str] = None
    trainer_name: Optional[str] = None


@router.post("/create-notebook")
async def create_notebook(http_request: Request, request: CreateNotebookRequest):
    """
    Create a new blank .ipynb notebook in the service account's Google Drive,
    share it (editor access) with the configured NOTEBOOK_SHARE_EMAIL,
    then load it into a session — no URL copy-paste needed.
    """
    import os
    ...
    try:
        ...
        created = drive_client.create_notebook(title=request.title or "Trainer Notebook")
        file_id = created["file_id"]
        notebook_url = created["url"]

        # 2. Share with the configured email (best-effort — don't fail session on share error)
        share_email = os.environ.get("NOTEBOOK_SHARE_EMAIL", "").strip()
        shared = False
        if share_email:
            shared = drive_client.share_file(file_id, share_email)
            if not shared:
                logger.warning(f"create_notebook: could not share {file_id} with {share_email}")

        # 3. Fetch the freshly created file content so it can be parsed into a session
        parsed, content_str = await notebook_parser.load_from_url(notebook_url)
        ...
        return {
            ...
            "shared_with": share_email if shared else None,
            "notebook": {
                "filename": parsed.filename,
                "metadata": parsed.metadata,
                "prompt": parsed.prompt,
        ...
        logger.exception("create_notebook failed")
        raise HTTPException(400, f"Failed to create notebook: {str(e)}")
...
def _normalize_review_keys(reviews: dict) -> dict:
    """
    Normalize review keys to a canonical format.
    Frontend sends mixed keys: row_N (submitted reviews with full data) and
    huntId (grading UI partial state). We prefer row_N keys (complete data)
    and remove bare huntId duplicates that are just partial grading state.
    """
    if not isinstance(reviews, dict):
        return reviews

    normalized = {}
    row_keys_hunt_ids = set()
    ...
    for key, val in reviews.items():
        ...
        if str(key).startswith("row_") and val.get("hunt_id"):
            row_keys_hunt_ids.add(str(val["hunt_id"]))

        normalized[key] = val

    # Remove bare huntId keys that are duplicates of row_N entries
    # (bare entries only have grades/notes/submitted, not the full review data)
    keys_to_remove = []
    for key in normalized:
        if str(key) in row_keys_hunt_ids and not str(key).startswith("row_"):
            val = normalized[key]
            if isinstance(val, dict) and not val.get("grading_basis") and not val.get("explanation"):
...
async def save_reviews(session_id: str, request: Request):
    """Save human reviews for notebook export. Persists to Redis so Submit-for-review sees them."""
    session = await _get_validated_session(session_id)

    review_status = await redis_store.get_review_status(session_id)
    if review_status in ("submitted", "approved", "rejected"):
        raise HTTPException(
            status_code=409,
            detail=f"Cannot modify reviews: task is '{review_status}'. Reviews are locked while under review or after final decision.",
        )
    ...
    merged = {**(session.human_reviews or {}), **reviews}
    session.human_reviews = _normalize_review_keys(merged)
    ...
    # Only invalidate QC on explicit saves (not auto-save drafts) to prevent
    # the Submit-for-Review button from being permanently disabled
    if not is_auto and await redis_store.get_qc_done(session_id):
        await redis_store.clear_qc_done(session_id)
        logger.info(f"Session {session_id}: QC invalidated — reviews modified after QC")
    if not is_auto:
...
async def save_snapshot(request: Request):
    ...
    Requires review_status = approved, or internal service auth via
    X-Internal-Secret header (preferred) or legacy X-Admin-Mode header.
    """
    try:
        from services.google_drive_client import drive_client
        ...
        is_internal = (
            (internal_secret and request.headers.get("X-Internal-Secret") == internal_secret)
            or request.headers.get("X-Admin-Mode") == "true"
        )
        if session_id and not is_internal:
            status = await redis_store.get_review_status(session_id)
            if status != "approved":
                raise HTTPException(
                    status_code=403,
                    detail=f"Colab save requires reviewer approval. Current status: '{status}'.",
                )
...
# ============== Write Pre-built Notebook to Drive ==============

@router.post("/write-notebook")
async def write_notebook(request: Request):
    """
    Write a fully pre-built notebook JSON directly to Google Drive.
    The caller is responsible for building all cell content.
    Body: { "url": "<drive url>", "notebook_json": { ...ipynb object... } }
    No reviewer-approval gate — this is trainer-initiated.
    """
    try:
        from services.google_drive_client import drive_client
        ...
        if not notebook_json:
            raise HTTPException(400, "Missing 'notebook_json' field")

        file_id = drive_client.get_file_id_from_url(url)
        if not file_id:
            raise HTTPException(400, f"Could not extract file_id from url: {url}")

        content = json.dumps(notebook_json, indent=2, ensure_ascii=False)
        success = drive_client.update_file_content(file_id, content)
        if not success:
