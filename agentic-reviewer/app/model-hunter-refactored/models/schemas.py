...
def _get_hunt_default(key: str, default: Any) -> Any:
    """Get hunt config default from global config if available."""
    try:
        from agentic_reviewer.config_loader import get_config_value
        h = get_config_value("hunt") or {}
...
class HuntConfig(BaseModel):
    ...
    parallel_workers: int = Field(default_factory=lambda: _get_hunt_default("parallel_workers_default", 4), ge=1, le=16)
    target_breaks: int = Field(default_factory=lambda: _get_hunt_default("target_breaks_default", 4), ge=1)
