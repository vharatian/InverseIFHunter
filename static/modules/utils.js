...
/**
 * Collect criterion IDs already present in text (C1:, C2 -, "id":"C1", etc.).
...
/**
 * Convert a criteria string (either structured "C1: ..." text OR JSON) into
 * a pretty-printed JSON string in the canonical criteria array format.
 * Returns the original string unchanged if it cannot be parsed.
 */
export function parseCriteriaToJSON(input) {
    ...
    try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
            const isValid = parsed.every(item =>
                ...
                Object.keys(item).some(k => k.startsWith('criteria'))
            );
            if (isValid) return JSON.stringify(parsed, null, 2);
        }
    } catch (_) { /* fall through to structured text */ }

    // Try structured "C1: description" text
    ...
    lines.forEach((line, index) => {
        ...
        if (match) {
            criteria.push({ id: `C${match[1]}`, [`criteria${match[1]}`]: match[2].trim() });
        } else {
            const n = index + 1;
            criteria.push({ id: `C${n}`, [`criteria${n}`]: line });
        }
    });

    if (criteria.length > 0) return JSON.stringify(criteria, null, 2);
