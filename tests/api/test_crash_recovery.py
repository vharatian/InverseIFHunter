...
class TestServerRestartDuringHunt:
    ...
    def test_completed_results_survive_session_reset(self, client, create_session):
        """After results are injected (simulating completed hunts), API retrieval works."""
        # Note: In the specific refactored architecture, the engine is stateless (Redis-backed).
        # "Resetting" the server just means a new request comes in.
        # Since inject_results_into_session writes to Redis, and the API reads from Redis,
        # this verifies that data persists across the request boundary without local state.
        sid = create_session
