...
# ---------------------------------------------------------------------------
# Note: Refactored architecture moved _persist_session from HuntEngine to
# helpers.shared. Tests now verify the shared helper directly.

@pytest.mark.unit
class TestHuntPersistence:
    """Verify _persist_session behavior (lives in helpers.shared in refactored design)."""

    @pytest.mark.asyncio
    async def test_persist_session_is_async(self):
        """_persist_session should be an async function."""
        import inspect
        from helpers.shared import _persist_session
        assert inspect.iscoroutinefunction(_persist_session)

    @pytest.mark.asyncio
    async def test_persist_session_callable(self, single_turn_session):
