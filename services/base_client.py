...
class BaseAPIClient(ABC):
    ...
    def __init__(self, api_key: Optional[str] = None, env_var_name: str = "API_KEY"):
        ...
        """
        self.api_key = api_key or self._resolve_api_key(env_var_name)
        self._client: Optional[httpx.AsyncClient] = None
        self._client_lock = asyncio.Lock()
        self._default_timeout = TIMEOUTS.get(self.PROVIDER_NAME, TIMEOUTS["default"])
    
    def _resolve_api_key(self, env_var_name: str) -> Optional[str]:
        """Resolve API key from global config, then os.getenv."""
        try:
            from agentic_reviewer.config_loader import get_config_value
            secrets = get_config_value("secrets") or {}
            config_key = env_var_name.lower()  # OPENROUTER_API_KEY -> openrouter_api_key
            val = secrets.get(config_key)
            if val:
                return val
        except Exception:
            pass
