...
class TaskSnapshot(BaseModel):
    ...
    reference: str = ""  # Raw response_reference text (criteria/rubric)
    ideal_response: str = ""  # Trainer-written expected/ideal response (notebook `response` field)
...
class ReviewResult(BaseModel):
    ...
    checkpoint: Checkpoint
    # Weighted score 0–100 (100 = all rules passed, lower = weighted failures).
    # Only meaningful when rule weights are configured; defaults to pass/fail binary.
    weighted_score: Optional[float] = Field(None, description="Weighted score 0–100, if weights are configured")
