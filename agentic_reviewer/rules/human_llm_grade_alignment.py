...
def _build_prompt(snapshot: TaskSnapshot) -> str:
    ...
    lines = [
        ...
        "",
        "TASK PROMPT:",
        snapshot.prompt if snapshot.prompt else "(none)",
        "",
    ...
    for i, hunt in enumerate(snapshot.selected_hunts[:4], 1):
        ...
        lines.append(f"LLM explanation: {hunt.judge_explanation or '(none)'}")
        if human:
            lines.append(f"Human grades: {human.grades}")
            lines.append(f"Human explanation: {human.explanation or '(none)'}")
        else:
...
def check_human_llm_grade_alignment(snapshot: TaskSnapshot, params: dict) -> ReviewIssue | None:
    ...
    for i, hunt in enumerate(snapshot.selected_hunts[:4], 1):
        ...
        slots.append({
            ...
            "model": hunt.model,
            "response_preview": hunt.response or "",
            "human_grades": human_grades if human else {},
            "human_explanation": (human.explanation or "") if human else "",
            "llm_judge_score": hunt.judge_score,
            "llm_judge_criteria": llm_criteria,
            "llm_judge_explanation": hunt.judge_explanation or "",
