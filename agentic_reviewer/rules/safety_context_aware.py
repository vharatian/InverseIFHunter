"""
Safety rule: optional zero-tolerance or context-aware mode.

Zero-tolerance (params.zero_tolerance=true): FAIL if prompt contains ANY reference to
prohibited content — explicit or implicit — including fiction, examples, hypotheticals,
metaphors, or academic framing.

Context-aware (default): PASS when prompt discusses or avoids prohibited topics;
FAIL only when prompt requests or encourages prohibited content.

Uses LLM Council. Prohibited list from config (params.prohibited_text) or default rubrics.
"""
...
def get_council_prompt(snapshot: TaskSnapshot, params: dict | None = None) -> str:
    ...
    zero_tolerance = params.get("zero_tolerance", False)
    return _build_prompt(snapshot, prohibited, zero_tolerance)


def _build_prompt(snapshot: TaskSnapshot, prohibited_text: str, zero_tolerance: bool = False) -> str:
    """Build prompt for council: zero-tolerance or context-aware safety check."""
    prompt = snapshot.prompt or ""
    lines = [
        "You are a safety reviewer. Check if the TASK PROMPT contains prohibited content.",
        "",
        "PROHIBITED CONTENT:",
        prohibited_text.strip(),
        "",
        "TASK PROMPT:",
    ...
    if zero_tolerance:
        lines.extend([
            ...
            "- FAIL if the prompt contains ANY reference to prohibited content — explicit OR implicit.",
            "- This includes fiction, examples, hypotheticals, metaphors, and academic framing. Any appearance = FAIL.",
            "- No exceptions. When in doubt, FAIL.",
            "",
        ])
    else:
        lines.extend([
...
def check_safety_context_aware(snapshot: TaskSnapshot, params: dict) -> ReviewIssue | None:
    ...
    try:
        prompt = _build_prompt(snapshot, prohibited, zero_tolerance)
    ...
    if zero_tolerance:
        msg = f"Council detected prohibited content (zero-tolerance). Votes: {vote_summary}"
        hint = "The prompt must contain NO references to prohibited content (explicit or implicit), including in fiction, examples, hypotheticals, or academic framing. Remove or rephrase."
    else:
        msg = f"Council detected prohibited content in prompt (requesting/encouraging). Votes: {vote_summary}"
    ...
    return ReviewIssue(
        ...
        details={
            ...
            "prompt_preview": snapshot.prompt or "",
        },
    )
